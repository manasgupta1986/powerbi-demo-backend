...
async function saveStateMeta(payload, savedSlices) {
  ...
  const persistedSlices = savedSlices.map((s) => ({
    sliceIndex: s.sliceIndex, scrollY: s.scrollY,
    viewportWidth: s.viewportWidth, viewportHeight: s.viewportHeight,
    totalScrollHeight: s.totalScrollHeight, clientHeight: s.clientHeight,
    capturedAt: s.capturedAt, imageFile: s.imageFile, imagePath: s.imagePath
  }));
  const stateMeta = {
    runId: payload.runId, stateId: payload.stateId, stepId: payload.stepId || "",
    ...
    sliceCount: persistedSlices.length, slices: persistedSlices
  };
...
async function analyzeStateWithAI(payload, stateMeta) {
  ...
  const content = [
    {
      ...
      text:
        ...
        `Return strict JSON with charts, datapoints, weekMappings, executiveSummary and limitations.\n` +
  ...
  for (const slice of representative) {
    if (slice?.imageDataUrl) content.push({ type: "image_url", image_url: { url: slice.imageDataUrl } });
  }
...
function buildHeuristicReport(latestData) {
  ...
  if (!rows.length) {
    return [
      ...
      `Stored states are available for review.`
    ].join("\n\n");
  }
  ...
  for (const [_key, groupRows] of groups.entries()) {
    ...
    if (ordered.length >= 2) {
      ...
      movementLines.push(
        `${last.seriesName || last.appName || "Series"} in ${last.chartTitle || "chart"} moved from ${first.value}${first.unit || ""} to ${last.value}${last.unit || ""} (${delta >= 0 ? "+" : ""}${delta}${last.unit || ""}).`
      );
    }
  }
  return [
    ...
    movementLines.length ? movementLines.slice(0, 8).join(" ") : `Not enough comparable numeric datapoints yet.`,
...
function buildAskFallback(latestData, question) {
  ...
  if (matchedRows.length) {
    ...
    return `Matching datapoints: ${sample.join("; ")}.`;
  }
  return rows.length
    ? `I could not find that in the extracted dataset alone.`
    : `No numeric extraction rows are available yet.`;
}
...
function buildCondensedDataset(latestData, limit = 350) {
  ...
  return {
    ...
    states: (latestData?.states || []).map((s) => ({
      pageName: s.pageName || "", stateType: s.stateType || "baseline",
      filterName: s.filterName || "", filterValue: s.filterValue || "",
      sliceCount: s.sliceCount || 0, capturedAt: s.capturedAt || ""
    })),
  ...
  try {
    const text = await callOpenAIChatText({
      ...
      messages: [
        {
          role: "system",
          content: "You write concise business analysis from structured dashboard data only. Do not use external knowledge. Use numeric values whenever available. If a number is approximate, say approximately. Frame the report from the named client's perspective."
        },
        {
          role: "user",
          content:
  ...
  try {
    const text = await callOpenAIChatText({
      ...
      messages: [
        {
          role: "system",
          content: "Answer questions using ONLY the provided dashboard dataset. Do not use external knowledge. If the answer is not in the data, say that clearly. Be numeric when possible."
        },
...
<head>
  <meta charset="utf-8" />
  <title>PowerBI Guided Capture Report</title>
  <style>
...
<body>
  ...
  <div class="card">
    ...
    <table>
      <thead><tr><th>State</th><th>Slices</th><th>Captured At</th></tr></thead>
  ...
  <div class="card">
    ...
    <table>
      <thead><tr><th>Page</th><th>State</th><th>Chart</th><th>Series / App</th><th>Week / X</th><th>Inferred Date</th><th>Value</th><th>Confidence</th></tr></thead>
      <tbody>${datapointRowsHtml}</tbody>
    </table>
  </div>
</body>
</html>`;
...
async function buildLatestDataForRun(runId) {
  const runMeta = (await readJsonIfExists(getRunMetaPath(runId), null)) || {
    runId, clientName: "", dashboardUrl: "", mode: "GUIDED_CAPTURE_MANUAL_SET_AUTO_SCROLL",
    startedAt: "", finishedAt: "", lastUpdatedAt: nowIso()
  };
  ...
  for (const folder of stateFolders) {
    ...
    states.push({
      ...
      pageName: stateMeta.pageName || "", stateType: stateMeta.stateType || "baseline",
      filterName: stateMeta.filterName || "", filterValue: stateMeta.filterValue || "",
      capturedAt: stateMeta.capturedAt || "", sliceCount: stateMeta.sliceCount || 0,
...
async function runExtractionInBackground(payload, stateMeta) {
  const runId = payload.runId; const stateId = payload.stateId;
  try {
    setExtractionStatus(runId, stateId, { stage: "extracting", progress: 20, message: "Sending screenshots to OpenAI for extraction" });
    let extraction;
    try {
...
app.post("/capture-state", async (req, res) => {
  ...
  setExtractionStatus(payload.runId, payload.stateId, { stage: "saving", progress: 5, message: "Saving screenshots and metadata" });
  ...
  setExtractionStatus(payload.runId, payload.stateId, { stage: "saved", progress: 15, message: "Screenshots saved. Extraction queued." });

  res.json({ ok: true, runId: payload.runId, stateId: payload.stateId, savedSlices: savedSlices.length, extractionAsync: true });

  const payloadForAI = { ...payload, slices: savedSlices };
  runExtractionInBackground(payloadForAI, stateMeta).catch((err) => {
    setExtractionStatus(payload.runId, payload.stateId, {
      stage: "failed", progress: 100,
      message: `Background extraction crashed: ${err.message}`,
      error: err.message, finishedAt: nowIso()
    });
  });
});
